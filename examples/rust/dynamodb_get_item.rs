...
// Uso: cargo run --bin dynamodb_get_item -- <tabla> <id>
...
async fn main() -> Result<(), aws_sdk_dynamodb::Error> {
    ...
    if args.len() < 3 {
        eprintln!("Uso: cargo run --bin dynamodb_get_item -- <tabla> <id>");
        std::process::exit(1);
    }
    ...
    match result.item() {
        None => println!("No existe ningun item con id=\"{id}\" en {table_name}."),
        Some(item) => {
            println!("Item encontrado:");
            println!("{item:#?}");
        }
    }
    Ok(())
