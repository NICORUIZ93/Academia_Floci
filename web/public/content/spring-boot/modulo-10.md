...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Dos microservicios Spring Boot comunicándose vía gateway con circuit breaker, más cuatro ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Config Server y service discovery | `src/main/java/io/academia/rutaflow/module-10/topic-1-config-server-y-service-discovery.java` | prueba + salida observable |
| 2. Spring Cloud Gateway | `src/main/java/io/academia/rutaflow/module-10/topic-2-spring-cloud-gateway.java` | prueba + salida observable |
| 3. Circuit breaker con Resilience4j | `src/main/java/io/academia/rutaflow/module-10/topic-3-circuit-breaker-con-resilience4j.java` | prueba + salida observable |
| 4. OAuth2/OIDC, Keycloak y Token Relay sin perder la frontera de autorización | `src/main/java/io/academia/rutaflow/module-10/topic-4-oauth2-oidc-keycloak-y-token-relay-sin-perder-la-front.java` | prueba + salida observable |
| 5. HTTP Interfaces, deadlines y descubrimiento según el entorno | `src/main/java/io/academia/rutaflow/module-10/topic-5-http-interfaces-deadlines-y-descubrimiento-segun-el-en.java` | prueba + salida observable |
| 6. DDD para decidir límites y propiedad de datos | `src/main/java/io/academia/rutaflow/module-10/topic-6-ddd-para-decidir-limites-y-propiedad-de-datos.java` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Dos microservicios Spring Boot comunicándose vía gateway con circuit breaker, más cuatro ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Envía una petición inválida o sustituye una dependencia por un fallo controlado; verifica estado, cuerpo y causa. Guarda en `evidence/module-10/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Microservicios con Spring Cloud** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
