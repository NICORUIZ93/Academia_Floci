...
Floci emula 52 servicios AWS en un solo puerto (`4566`). Todos los servicios utilizan el protocolo de conexión AWS real, sus comandos AWS CLI existentes y sus clientes SDK funcionan sin modificaciones.
...
| [IAM](iam.md) | `POST /` con parámetro `Action=` | Consulta | 68 |
...
| [EC2](ec2.md) | `POST /` con parámetro `Action=` | Consulta EC2 | 61 |
...
| [Grupos de recursos que etiquetan API](resource-groups-tagging.md) | `POST /` + `X-Amz-Target: ResourceGroupsTaggingAPI_20170126.*` | JSON 1.1 | 5 |
