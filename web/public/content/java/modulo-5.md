...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Servicio concurrente que procesa N tareas en paralelo usando virtual threads, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. ExecutorService y gestión de hilos | `src/main/java/academy/module-5/topic-1-executorservice-y-gestion-de-hilos.java` | prueba + salida observable |
| 2. CompletableFuture | `src/main/java/academy/module-5/topic-2-completablefuture.java` | prueba + salida observable |
| 3. Virtual threads | `src/main/java/academy/module-5/topic-3-virtual-threads.java` | prueba + salida observable |
| 4. Condiciones de carrera y sincronización | `src/main/java/academy/module-5/topic-4-condiciones-de-carrera-y-sincronizacion.java` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Servicio concurrente que procesa N tareas en paralelo usando virtual threads, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Viola una precondición o usa un valor frontera; la prueba debe expresar la regla incumplida. Guarda en `evidence/module-5/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Concurrencia — hilos y virtual threads** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
