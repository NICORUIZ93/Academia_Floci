...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un laboratorio que crea un usuario con permisos mínimos verificados con el simulador de políticas, y tres ejercicios de evaluación sobre diseño de políticas, roles frente a usuarios, y diagnóstico de permisos.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Principio de mínimo privilegio | `infra/module-7/topic-1-principio-de-minimo-privilegio.tf` | prueba + salida observable |
| 2. Modelo de responsabilidad compartida | `infra/module-7/topic-2-modelo-de-responsabilidad-compartida.tf` | prueba + salida observable |
| 3. Usuarios, grupos y roles | `infra/module-7/topic-3-usuarios-grupos-y-roles.tf` | prueba + salida observable |
| 4. Estructura de una política — acciones, recursos y condiciones | `infra/module-7/topic-4-estructura-de-una-politica-acciones-recursos-y-condici.tf` | prueba + salida observable |
| 5. Buenas prácticas — roles sobre usuarios, políticas restrictivas, rotación de credenciales | `infra/module-7/topic-5-buenas-practicas-roles-sobre-usuarios-politicas-restri.tf` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un laboratorio que crea un usuario con permisos mínimos verificados con el simulador de políticas, y tres ejercicios de evaluación sobre diseño de políticas, roles frente a usuarios, y diagnóstico de permisos.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-7/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Identidad y acceso con IAM** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
