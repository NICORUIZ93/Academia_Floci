...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Dos laboratorios (uno por proveedor) y tres ejercicios de evaluación sobre equivalencias de servicios y criterios de elección entre proveedores.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. floci-az — Blob Storage, Queue Storage, Table Storage, Cosmos DB, Functions | `infra/module-8/topic-1-floci-az-blob-storage-queue-storage-table-storage-cosm.tf` | prueba + salida observable |
| 2. floci-gcp — Cloud Storage, Pub/Sub, Firestore, Cloud Functions | `infra/module-8/topic-2-floci-gcp-cloud-storage-pub-sub-firestore-cloud-functi.tf` | prueba + salida observable |
| 3. Comparativa AWS vs Azure vs GCP por categoría de servicio | `infra/module-8/topic-3-comparativa-aws-vs-azure-vs-gcp-por-categoria-de-servi.tf` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Dos laboratorios (uno por proveedor) y tres ejercicios de evaluación sobre equivalencias de servicios y criterios de elección entre proveedores.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-8/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Azure y GCP con Floci** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
