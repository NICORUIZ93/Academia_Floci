...
|---|---|---|
| 1. ELB v2 — balanceadores, grupos objetivo y reglas | `docs/decisions/module-22-topic-1.md` | contexto + alternativas + decisión + consecuencias |
| 2. ACM — certificados TLS con criptografía real | `docs/decisions/module-22-topic-2.md` | contexto + alternativas + decisión + consecuencias |
| 3. CloudFront — distribución de contenido y control de acceso al origen | `docs/decisions/module-22-topic-3.md` | contexto + alternativas + decisión + consecuencias |
| 4. Route53 — zonas alojadas y registros de recursos | `docs/decisions/module-22-topic-4.md` | contexto + alternativas + decisión + consecuencias |
| 5. Cómo se integran los cuatro servicios en una arquitectura de borde real | `docs/decisions/module-22-topic-5.md` | contexto + alternativas + decisión + consecuencias |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Dos laboratorios prácticos (balanceador con grupo objetivo, y certificado + distribución + zona DNS) y tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-22/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Balanceo de carga, CDN y DNS — ELB, CloudFront, Route53 y ACM** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
