...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un laboratorio práctico (crear un servidor, un usuario y una clave SSH, y controlar el ciclo de vida del servidor) y tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Qué resuelve Transfer Family | `docs/decisions/module-30-topic-1.md` | contexto + alternativas + decisión + consecuencias |
| 2. Ciclo de vida del servidor y modelo de usuarios | `docs/decisions/module-30-topic-2.md` | contexto + alternativas + decisión + consecuencias |
| 3. Claves públicas SSH y autenticación de usuarios | `docs/decisions/module-30-topic-3.md` | contexto + alternativas + decisión + consecuencias |
| 4. Los límites de la Fase 1 — plano de gestión completo, plano de datos pendiente | `docs/decisions/module-30-topic-4.md` | contexto + alternativas + decisión + consecuencias |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un laboratorio práctico (crear un servidor, un usuario y una clave SSH, y controlar el ciclo de vida del servidor) y tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-30/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Transferencia de archivos gestionada con Transfer Family** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
