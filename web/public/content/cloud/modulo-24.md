...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Dos laboratorios prácticos (una compilación real con CodeBuild, y un despliegue canary de Lambda con CodeDeploy) y tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. CodeBuild — compilaciones reales dentro de contenedores Docker | `docs/decisions/module-24-topic-1.md` | contexto + alternativas + decisión + consecuencias |
| 2. buildspec.yml — fases y artefactos | `docs/decisions/module-24-topic-2.md` | contexto + alternativas + decisión + consecuencias |
| 3. CodeDeploy — aplicaciones, grupos y configuraciones predefinidas | `docs/decisions/module-24-topic-3.md` | contexto + alternativas + decisión + consecuencias |
| 4. Despliegue Blue/Green de Lambda — cambio de tráfico por alias | `docs/decisions/module-24-topic-4.md` | contexto + alternativas + decisión + consecuencias |
| 5. Despliegue Blue/Green de ECS — cambio de tráfico por listener ELB | `docs/decisions/module-24-topic-5.md` | contexto + alternativas + decisión + consecuencias |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Dos laboratorios prácticos (una compilación real con CodeBuild, y un despliegue canary de Lambda con CodeDeploy) y tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-24/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **CI/CD nativo de AWS con CodeBuild y CodeDeploy** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
