...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un laboratorio práctico (crear un clúster, leer/escribir con redis-cli, y crear un usuario con autenticación IAM) y tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Qué resuelve un caché en memoria — y cuándo no ayuda | `docs/decisions/module-23-topic-1.md` | contexto + alternativas + decisión + consecuencias |
| 2. Arquitectura de ElastiCache en Floci — contenedores reales, no simulación | `docs/decisions/module-23-topic-2.md` | contexto + alternativas + decisión + consecuencias |
| 3. Creación de clústeres y conexión con clientes estándar | `docs/decisions/module-23-topic-3.md` | contexto + alternativas + decisión + consecuencias |
| 4. Autenticación IAM para el plano de datos de ElastiCache | `docs/decisions/module-23-topic-4.md` | contexto + alternativas + decisión + consecuencias |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un laboratorio práctico (crear un clúster, leer/escribir con redis-cli, y crear un usuario con autenticación IAM) y tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-23/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Caché en memoria con ElastiCache** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
