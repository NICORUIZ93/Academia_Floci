...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **API completa con los mismos endpoints funcionando en AWS local, Azure local y GCP local, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Arquitectura multi-nube y portabilidad de conocimiento | `infra/module-31/topic-1-arquitectura-multi-nube-y-portabilidad-de-conocimiento.tf` | prueba + salida observable |
| 2. Arquitectura interna de cloud local y su uso en CI/CD | `infra/module-31/topic-2-arquitectura-interna-de-cloud-local-y-su-uso-en-ci-cd.tf` | prueba + salida observable |
| 3. Migración desde otros emuladores, y límites de un emulador | `infra/module-31/topic-3-migracion-desde-otros-emuladores-y-limites-de-un-emula.tf` | prueba + salida observable |
| 4. Testcontainers — pruebas de integración automatizadas | `infra/module-31/topic-4-testcontainers-pruebas-de-integracion-automatizadas.tf` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **API completa con los mismos endpoints funcionando en AWS local, Azure local y GCP local, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-31/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Proyecto integrador: API multi-nube con AWS, Azure y GCP** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
