...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Aplicación que lee todos sus secretos y configuración desde la nube, sin nada hardcodeado, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Secrets Manager y por qué no usar variables de entorno hardcodeadas | `infra/module-10/topic-1-secrets-manager-y-por-que-no-usar-variables-de-entorno.tf` | prueba + salida observable |
| 2. KMS y envelope encryption | `infra/module-10/topic-2-kms-y-envelope-encryption.tf` | prueba + salida observable |
| 3. SSM Parameter Store vs Secrets Manager | `infra/module-10/topic-3-ssm-parameter-store-vs-secrets-manager.tf` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Aplicación que lee todos sus secretos y configuración desde la nube, sin nada hardcodeado, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-10/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Secretos y configuración: Secrets Manager, Key Vault y Secret Manager** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
