...
|---|---|---|
| 1. Bases de datos de grafos — cuando las relaciones son el dato | `docs/decisions/module-28-topic-1.md` | contexto + alternativas + decisión + consecuencias |
| 2. Neptune en Floci — un servidor Gremlin real, no una simulación | `docs/decisions/module-28-topic-2.md` | contexto + alternativas + decisión + consecuencias |
| 3. OpenSearch — modo simulado y modo real | `docs/decisions/module-28-topic-3.md` | contexto + alternativas + decisión + consecuencias |
| 4. Eligiendo entre Neptune, OpenSearch y DynamoDB | `docs/decisions/module-28-topic-4.md` | contexto + alternativas + decisión + consecuencias |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Dos laboratorios prácticos (un grafo de relaciones con Neptune, y un dominio OpenSearch en modo real) y tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-28/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Bases de datos de grafos y búsqueda — Neptune y OpenSearch** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
