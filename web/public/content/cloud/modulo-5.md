...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Dos laboratorios (desplegar/invocar una función, y actualizar su código y configuración) y tres ejercicios de evaluación sobre el contrato de una función, statelessness, y triggers.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Qué es serverless — ventajas y desventajas | `infra/module-5/topic-1-que-es-serverless-ventajas-y-desventajas.tf` | prueba + salida observable |
| 2. Estructura de una función Lambda | `infra/module-5/topic-2-estructura-de-una-funcion-lambda.tf` | prueba + salida observable |
| 3. Runtimes — Node.js, Python, Java, Go | `infra/module-5/topic-3-runtimes-node-js-python-java-go.tf` | prueba + salida observable |
| 4. Payload de entrada y respuesta | `infra/module-5/topic-4-payload-de-entrada-y-respuesta.tf` | prueba + salida observable |
| 5. Versionado y alias | `infra/module-5/topic-5-versionado-y-alias.tf` | prueba + salida observable |
| 6. Integración con S3, DynamoDB Streams y API Gateway | `infra/module-5/topic-6-integracion-con-s3-dynamodb-streams-y-api-gateway.tf` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Dos laboratorios (desplegar/invocar una función, y actualizar su código y configuración) y tres ejercicios de evaluación sobre el contrato de una función, statelessness, y triggers.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Cambia un endpoint, permiso o identificador por un valor inválido; inspecciona la respuesta del emulador antes de corregir. Guarda en `evidence/module-5/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Serverless con Lambda** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
