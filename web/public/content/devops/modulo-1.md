...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un laboratorio que limpia un historial con rebase, provoca y resuelve un conflicto, y usa `bisect` para encontrar una regresión; tres ejercicios de evaluación sobre elección de estrategia de branching, riesgos de reescribir historial compartido, y diferencias entre `reset` y `revert`.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Trunk-based development vs GitFlow | `infra/module-1/topic-1-trunk-based-development-vs-gitflow.yaml` | prueba + salida observable |
| 2. Rebase interactivo vs merge | `infra/module-1/topic-2-rebase-interactivo-vs-merge.yaml` | prueba + salida observable |
| 3. Bisect para encontrar regresiones | `infra/module-1/topic-3-bisect-para-encontrar-regresiones.yaml` | prueba + salida observable |
| 4. Hooks de Git y commits firmados | `infra/module-1/topic-4-hooks-de-git-y-commits-firmados.yaml` | prueba + salida observable |
| 5. Monorepos vs polyrepos | `infra/module-1/topic-5-monorepos-vs-polyrepos.yaml` | prueba + salida observable |
| 6. git cherry-pick, git stash, git reset vs git revert | `infra/module-1/topic-6-git-cherry-pick-git-stash-git-reset-vs-git-revert.yaml` | prueba + salida observable |
| 7. git reflog, git worktree y git submodule | `infra/module-1/topic-7-git-reflog-git-worktree-y-git-submodule.yaml` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un laboratorio que limpia un historial con rebase, provoca y resuelve un conflicto, y usa `bisect` para encontrar una regresión; tres ejercicios de evaluación sobre elección de estrategia de branching, riesgos de reescribir historial compartido, y diferencias entre `reset` y `revert`.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Rompe una referencia, variable o healthcheck y localiza la causa con la validación o los logs. Guarda en `evidence/module-1/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Git avanzado y estrategias de branching** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
