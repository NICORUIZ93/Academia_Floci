...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un laboratorio que expone una métrica propia, la consulta con PromQL, la visualiza en Grafana y configura una alerta, y tres ejercicios de evaluación sobre elección de tipo de métrica, diseño de SLO, y valor de negocio de las métricas DORA.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Modelo de métricas de Prometheus — counter, gauge, histogram | `infra/module-9/topic-1-modelo-de-metricas-de-prometheus-counter-gauge-histogr.yaml` | prueba + salida observable |
| 2. PromQL esencial | `infra/module-9/topic-2-promql-esencial.yaml` | prueba + salida observable |
| 3. Dashboards en Grafana | `infra/module-9/topic-3-dashboards-en-grafana.yaml` | prueba + salida observable |
| 4. Alertmanager y reglas de alerta | `infra/module-9/topic-4-alertmanager-y-reglas-de-alerta.yaml` | prueba + salida observable |
| 5. SLI, SLO, SLA | `infra/module-9/topic-5-sli-slo-sla.yaml` | prueba + salida observable |
| 6. OpenTelemetry como estándar de instrumentación | `infra/module-9/topic-6-opentelemetry-como-estandar-de-instrumentacion.yaml` | prueba + salida observable |
| 7. Métricas DORA — Lead Time, Deployment Frequency, MTTR, Change Failure Rate | `infra/module-9/topic-7-metricas-dora-lead-time-deployment-frequency-mttr-chan.yaml` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un laboratorio que expone una métrica propia, la consulta con PromQL, la visualiza en Grafana y configura una alerta, y tres ejercicios de evaluación sobre elección de tipo de métrica, diseño de SLO, y valor de negocio de las métricas DORA.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Rompe una referencia, variable o healthcheck y localiza la causa con la validación o los logs. Guarda en `evidence/module-9/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Observabilidad con Prometheus y Grafana** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
...
### Ejemplo guiado: SRE (SLI/SLO/SLA)
