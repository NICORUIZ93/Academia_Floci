...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **App reestructurada en capas (vista/viewmodel/datos) con dependencias inyectadas, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. De una vista "gorda" a MVVM | `Features/module-8/topic-1-de-una-vista-gorda-a-mvvm.swift` | prueba + salida observable |
| 2. Capas del proyecto e inyección por inicializador | `Features/module-8/topic-2-capas-del-proyecto-e-inyeccion-por-inicializador.swift` | prueba + salida observable |
| 3. Cuándo MVVM no alcanza | `Features/module-8/topic-3-cuando-mvvm-no-alcanza.swift` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **App reestructurada en capas (vista/viewmodel/datos) con dependencias inyectadas, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula permiso denegado, respuesta vacía o tarea cancelada; verifica estado y mensaje. SwiftUI requiere macOS. Guarda en `evidence/module-8/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Arquitectura MVVM** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
