...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Implementación de un contador privado y un módulo con estado usando closures, más tres ejercicios de evaluación sobre closures, TDZ y `this`.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Scope léxico y closures | `src/module-2/topic-1-scope-lexico-y-closures.ts` | prueba + salida observable |
| 2. Hoisting y Temporal Dead Zone | `src/module-2/topic-2-hoisting-y-temporal-dead-zone.ts` | prueba + salida observable |
| 3. Call stack y contexto de ejecución | `src/module-2/topic-3-call-stack-y-contexto-de-ejecucion.ts` | prueba + salida observable |
| 4. this según el modo de invocación | `src/module-2/topic-4-this-segun-el-modo-de-invocacion.ts` | prueba + salida observable |
| 5. Module pattern y factory functions | `src/module-2/topic-5-module-pattern-y-factory-functions.ts` | prueba + salida observable |
| 6. Execution Context, Scope Chain y Lexical Environment | `src/module-2/topic-6-execution-context-scope-chain-y-lexical-environment.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Implementación de un contador privado y un módulo con estado usando closures, más tres ejercicios de evaluación sobre closures, TDZ y `this`.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Prueba un valor límite, un tipo inesperado o una operación fuera de orden; compara la salida con tu predicción. Guarda en `evidence/module-2/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Scope, closures y el modelo de ejecución** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
