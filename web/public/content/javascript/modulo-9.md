...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Una suite de pruebas con cobertura mayor al 80% sobre la biblioteca de funciones del Módulo 1, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Unit testing con Vitest | `src/module-9/topic-1-unit-testing-con-vitest.ts` | prueba + salida observable |
| 2. Mocks, spies y fakes | `src/module-9/topic-2-mocks-spies-y-fakes.ts` | prueba + salida observable |
| 3. Mockear fetch | `src/module-9/topic-3-mockear-fetch.ts` | prueba + salida observable |
| 4. ESLint, Prettier y cobertura de código | `src/module-9/topic-4-eslint-prettier-y-cobertura-de-codigo.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Una suite de pruebas con cobertura mayor al 80% sobre la biblioteca de funciones del Módulo 1, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Prueba un valor límite, un tipo inesperado o una operación fuera de orden; compara la salida con tu predicción. Guarda en `evidence/module-9/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Testing y calidad de código** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
