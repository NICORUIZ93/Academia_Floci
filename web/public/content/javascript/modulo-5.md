...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un diagrama y una demo ejecutable que predicen el orden exacto de ejecución de un script asíncrono, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. El Event Loop | `src/module-5/topic-1-el-event-loop.ts` | prueba + salida observable |
| 2. Promesas | `src/module-5/topic-2-promesas.ts` | prueba + salida observable |
| 3. Promise.all, allSettled, race y any | `src/module-5/topic-3-promise-all-allsettled-race-y-any.ts` | prueba + salida observable |
| 4. El motor V8 — JIT y AST | `src/module-5/topic-4-el-motor-v8-jit-y-ast.ts` | prueba + salida observable |
| 5. Stack, Heap y Garbage Collection | `src/module-5/topic-5-stack-heap-y-garbage-collection.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un diagrama y una demo ejecutable que predicen el orden exacto de ejecución de un script asíncrono, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Prueba un valor límite, un tipo inesperado o una operación fuera de orden; compara la salida con tu predicción. Guarda en `evidence/module-5/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Asincronía I — Event Loop y Promesas** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
