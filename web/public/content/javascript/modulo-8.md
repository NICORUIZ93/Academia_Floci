...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un componente de UI interactivo (lista filtrable) sin frameworks, solo DOM API, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Selección y manipulación del DOM | `src/module-8/topic-1-seleccion-y-manipulacion-del-dom.ts` | prueba + salida observable |
| 2. Delegación de eventos | `src/module-8/topic-2-delegacion-de-eventos.ts` | prueba + salida observable |
| 3. Formularios y validación nativa | `src/module-8/topic-3-formularios-y-validacion-nativa.ts` | prueba + salida observable |
| 4. Web APIs — localStorage e IntersectionObserver | `src/module-8/topic-4-web-apis-localstorage-e-intersectionobserver.ts` | prueba + salida observable |
| 5. ResizeObserver y MutationObserver | `src/module-8/topic-5-resizeobserver-y-mutationobserver.ts` | prueba + salida observable |
| 6. requestAnimationFrame y requestIdleCallback | `src/module-8/topic-6-requestanimationframe-y-requestidlecallback.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un componente de UI interactivo (lista filtrable) sin frameworks, solo DOM API, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Prueba un valor límite, un tipo inesperado o una operación fuera de orden; compara la salida con tu predicción. Guarda en `evidence/module-8/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **El DOM y eventos del navegador** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
