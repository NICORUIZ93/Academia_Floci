...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Una suite de pruebas de integración que levanta la API contra una base de datos real en Docker, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Vitest y Supertest — pruebas de integración HTTP reales | `src/module-7/topic-1-vitest-y-supertest-pruebas-de-integracion-http-reales.ts` | prueba + salida observable |
| 2. Testcontainers — bases de datos de prueba reales y efímeras | `src/module-7/topic-2-testcontainers-bases-de-datos-de-prueba-reales-y-efime.ts` | prueba + salida observable |
| 3. Mocks de servicios externos y CI | `src/module-7/topic-3-mocks-de-servicios-externos-y-ci.ts` | prueba + salida observable |
| 4. Alternativas de testing y debugging | `src/module-7/topic-4-alternativas-de-testing-y-debugging.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Una suite de pruebas de integración que levanta la API contra una base de datos real en Docker, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Envía una entrada inválida o desconecta una dependencia; verifica estado HTTP, cuerpo y log con contexto. Guarda en `evidence/module-7/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Testing e integración continua** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
