...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Una API con rate limiting, cabeceras seguras y validación estricta de entrada, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Cabeceras seguras con helmet | `src/module-10/topic-1-cabeceras-seguras-con-helmet.ts` | prueba + salida observable |
| 2. Rate limiting | `src/module-10/topic-2-rate-limiting.ts` | prueba + salida observable |
| 3. Inyección SQL y sanitización contra XSS | `src/module-10/topic-3-inyeccion-sql-y-sanitizacion-contra-xss.ts` | prueba + salida observable |
| 4. OWASP API Security Top 10 y auditoría de dependencias | `src/module-10/topic-4-owasp-api-security-top-10-y-auditoria-de-dependencias.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Una API con rate limiting, cabeceras seguras y validación estricta de entrada, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Envía una entrada inválida o desconecta una dependencia; verifica estado HTTP, cuerpo y log con contexto. Guarda en `evidence/module-10/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Seguridad en Node** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
