...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Construcción completa de la aplicación de gestión de tareas descrita, más tres ejercicios de evaluación de cierre.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Estructura del proyecto integrador | `src/app/features/module-13/topic-1-estructura-del-proyecto-integrador.ts` | prueba + salida observable |
| 2. Integrando routing, store y formularios | `src/app/features/module-13/topic-2-integrando-routing-store-y-formularios.ts` | prueba + salida observable |
| 3. TareasStore — combinando signals, computed y HttpClient | `src/app/features/module-13/topic-3-tareasstore-combinando-signals-computed-y-httpclient.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Construcción completa de la aplicación de gestión de tareas descrita, más tres ejercicios de evaluación de cierre.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula un estado vacío o un error HTTP y comprueba que la interfaz muestre recuperación y no una pantalla ambigua. Guarda en `evidence/module-13/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Proyecto integrador — aplicación standalone completa** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
