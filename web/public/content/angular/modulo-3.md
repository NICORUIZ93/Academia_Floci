...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un servicio compartido de estado consumido por al menos tres componentes distintos, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. @Injectable y providedIn: root | `src/app/features/module-3/topic-1-injectable-y-providedin-root.ts` | prueba + salida observable |
| 2. inject() frente a inyección por constructor | `src/app/features/module-3/topic-2-inject-frente-a-inyeccion-por-constructor.ts` | prueba + salida observable |
| 3. Jerarquía de inyectores | `src/app/features/module-3/topic-3-jerarquia-de-inyectores.ts` | prueba + salida observable |
| 4. Tokens de inyección y decoradores de resolución | `src/app/features/module-3/topic-4-tokens-de-inyeccion-y-decoradores-de-resolucion.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un servicio compartido de estado consumido por al menos tres componentes distintos, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula un estado vacío o un error HTTP y comprueba que la interfaz muestre recuperación y no una pantalla ambigua. Guarda en `evidence/module-3/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Servicios e inyección de dependencias** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
