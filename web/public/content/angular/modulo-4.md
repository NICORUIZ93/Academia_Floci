...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Una aplicación con al menos 4 rutas, una protegida por guard y una cargada de forma perezosa, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Router config con rutas standalone | `src/app/features/module-4/topic-1-router-config-con-rutas-standalone.ts` | prueba + salida observable |
| 2. Guards funcionales | `src/app/features/module-4/topic-2-guards-funcionales.ts` | prueba + salida observable |
| 3. Lazy loading con loadComponent | `src/app/features/module-4/topic-3-lazy-loading-con-loadcomponent.ts` | prueba + salida observable |
| 4. Parámetros de ruta, query params y capacidades avanzadas de routing | `src/app/features/module-4/topic-4-parametros-de-ruta-query-params-y-capacidades-avanzada.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Una aplicación con al menos 4 rutas, una protegida por guard y una cargada de forma perezosa, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula un estado vacío o un error HTTP y comprueba que la interfaz muestre recuperación y no una pantalla ambigua. Guarda en `evidence/module-4/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Routing y navegación** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
