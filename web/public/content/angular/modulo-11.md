...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Configuración de SSR con hidratación y un bloque `@defer` con trigger apropiado, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Server-Side Rendering | `src/app/features/module-11/topic-1-server-side-rendering.ts` | prueba + salida observable |
| 2. Hidratación | `src/app/features/module-11/topic-2-hidratacion.ts` | prueba + salida observable |
| 3. @defer — carga diferida de vistas | `src/app/features/module-11/topic-3-defer-carga-diferida-de-vistas.ts` | prueba + salida observable |
| 4. Zoneless | `src/app/features/module-11/topic-4-zoneless.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Configuración de SSR con hidratación y un bloque `@defer` con trigger apropiado, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula un estado vacío o un error HTTP y comprueba que la interfaz muestre recuperación y no una pantalla ambigua. Guarda en `evidence/module-11/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Performance, SSR y zoneless** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
