...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Planificación de una migración concreta usando update.angular.io, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Los saltos que importan | `src/app/features/module-12/topic-1-los-saltos-que-importan.ts` | prueba + salida observable |
| 2. Cómo leer un Angular Update Guide | `src/app/features/module-12/topic-2-como-leer-un-angular-update-guide.ts` | prueba + salida observable |
| 3. Por qué migrar una versión mayor a la vez | `src/app/features/module-12/topic-3-por-que-migrar-una-version-mayor-a-la-vez.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Planificación de una migración concreta usando update.angular.io, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula un estado vacío o un error HTTP y comprueba que la interfaz muestre recuperación y no una pantalla ambigua. Guarda en `evidence/module-12/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Apéndice: qué cambió entre versiones mayores** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
