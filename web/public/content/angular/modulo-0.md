...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Un proyecto Angular nuevo con un componente propio renderizando datos dinámicos, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. El CLI ya no genera NgModules | `src/app/features/module-0/topic-1-el-cli-ya-no-genera-ngmodules.ts` | prueba + salida observable |
| 2. Interpolación y property binding | `src/app/features/module-0/topic-2-interpolacion-y-property-binding.ts` | prueba + salida observable |
| 3. TypeScript a fondo — unknown, any, never y utility types | `src/app/features/module-0/topic-3-typescript-a-fondo-unknown-any-never-y-utility-types.ts` | prueba + salida observable |
| 4. Compilador AOT frente a JIT | `src/app/features/module-0/topic-4-compilador-aot-frente-a-jit.ts` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Un proyecto Angular nuevo con un componente propio renderizando datos dinámicos, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula un estado vacío o un error HTTP y comprueba que la interfaz muestre recuperación y no una pantalla ambigua. Guarda en `evidence/module-0/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Fundamentos y Angular CLI** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
