...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **La evidencia será una carpeta creada desde la terminal que contenga un programa mínimo y un `README.md`. El README deberá registrar los comandos utilizados, la salida correcta, un error provocado deliberadamente y la explicación de cómo se corrigió.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Del hardware al programa en ejecución | `src/module-0/topic-1-del-hardware-al-programa-en-ejecucion.py` | prueba + salida observable |
| 2. Archivos, carpetas y rutas sin perderse | `src/module-0/topic-2-archivos-carpetas-y-rutas-sin-perderse.py` | prueba + salida observable |
| 3. Cómo leer un comando antes de ejecutarlo | `src/module-0/topic-3-como-leer-un-comando-antes-de-ejecutarlo.py` | prueba + salida observable |
| 4. Primer programa, primer error y primera evidencia | `src/module-0/topic-4-primer-programa-primer-error-y-primera-evidencia.py` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **La evidencia será una carpeta creada desde la terminal que contenga un programa mínimo y un `README.md`. El README deberá registrar los comandos utilizados, la salida correcta, un error provocado deliberadamente y la explicación de cómo se corrigió.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Usa una entrada inválida o elimina una precondición y conserva el mensaje que explica la causa. Guarda en `evidence/module-0/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Cómo funciona tu entorno de desarrollo** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
