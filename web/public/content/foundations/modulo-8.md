...
|---|---|---|
| 1. El sistema operativo como administrador y frontera | `src/module-8/topic-1-el-sistema-operativo-como-administrador-y-frontera.py` | prueba + salida observable |
| 2. Memoria y concurrencia sin magia | `src/module-8/topic-2-memoria-y-concurrencia-sin-magia.py` | prueba + salida observable |
| 3. Linux como entorno observable | `src/module-8/topic-3-linux-como-entorno-observable.py` | prueba + salida observable |
| 4. Contenedores: aislamiento reproducible | `src/module-8/topic-4-contenedores-aislamiento-reproducible.py` | prueba + salida observable |
...
Usa una entrada inválida o elimina una precondición y conserva el mensaje que explica la causa. Guarda en `evidence/module-8/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Módulo 8** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
