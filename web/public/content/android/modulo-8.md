...
|---|---|---|
| 1. CoroutineWorker y garantía de ejecución | `app/src/main/java/academy/module-8/topic-1-coroutineworker-y-garantia-de-ejecucion.kt` | prueba + salida observable |
| 2. Constraints y trabajo periódico | `app/src/main/java/academy/module-8/topic-2-constraints-y-trabajo-periodico.kt` | prueba + salida observable |
| 3. Notificaciones desde background work | `app/src/main/java/academy/module-8/topic-3-notificaciones-desde-background-work.kt` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Tarea periódica con WorkManager que sincroniza datos con constraints de red, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula permiso denegado, proceso recreado o dato ausente; verifica que la pantalla conserve un estado comprensible. Guarda en `evidence/module-8/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Trabajo en segundo plano** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
...
### Ejemplo guiado: Notificaciones (push notifications)
...
class NotificacionesPushNotificationsUseCase {
    operator fun invoke(): Evidence = Evidence(topic = "Notificaciones (push notifications)", passed = true)
}
```
