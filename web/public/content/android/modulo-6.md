...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **App con caché local en Room que funciona sin conexión a internet, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. Entities, DAOs y Database | `app/src/main/java/academy/module-6/topic-1-entities-daos-y-database.kt` | prueba + salida observable |
| 2. Room + Flow reactivo, y migraciones | `app/src/main/java/academy/module-6/topic-2-room-flow-reactivo-y-migraciones.kt` | prueba + salida observable |
| 3. Estrategia offline-first | `app/src/main/java/academy/module-6/topic-3-estrategia-offline-first.kt` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **App con caché local en Room que funciona sin conexión a internet, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula permiso denegado, proceso recreado o dato ausente; verifica que la pantalla conserve un estado comprensible. Guarda en `evidence/module-6/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Persistencia local con Room** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
