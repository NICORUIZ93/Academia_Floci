...
Este recorrido parte de una carpeta vacía. Al finalizar tendrás **Pantalla con UDF completo: eventos de usuario → `ViewModel` → `StateFlow` → UI, más tres ejercicios de evaluación.** No avances ejecutando comandos que no comprendes: primero identifica la entrada, la transformación y la evidencia que comprobará el resultado.
...
|---|---|---|
| 1. StateFlow en el ViewModel | `app/src/main/java/academy/module-4/topic-1-stateflow-en-el-viewmodel.kt` | prueba + salida observable |
| 2. collectAsStateWithLifecycle y UDF | `app/src/main/java/academy/module-4/topic-2-collectasstatewithlifecycle-y-udf.kt` | prueba + salida observable |
| 3. SharedFlow para eventos de un solo uso | `app/src/main/java/academy/module-4/topic-3-sharedflow-para-eventos-de-un-solo-uso.kt` | prueba + salida observable |
...
**Resultado esperado:** el comando reconoce el proyecto y termina sin errores antes de introducir el cambio del capítulo. Después del incremento, la evidencia debe demostrar: **Pantalla con UDF completo: eventos de usuario → `ViewModel` → `StateFlow` → UI, más tres ejercicios de evaluación.**

Si falla la línea base, no continúes. Localiza el primer mensaje que indique archivo, línea o dependencia; formula una causa y compruébala con un cambio pequeño.

### 5. Provoca un fallo y recupérate

Simula permiso denegado, proceso recreado o dato ausente; verifica que la pantalla conserve un estado comprensible. Guarda en `evidence/module-4/` el comando, la salida relevante, tu hipótesis y la corrección. Revierte únicamente el cambio deliberado; no borres todo el proyecto para ocultar la causa.

### 6. Conecta el capítulo con RutaFlow

Aplica el aprendizaje de **Estado con StateFlow y Compose** a un incremento vertical de RutaFlow. Define qué componente produce el dato, qué contrato lo transporta, quién lo consume y cómo observarás un fallo. La entrega final incluye archivo o decisión, prueba, salida, error corregido y una limitación que todavía validarías en producción.
