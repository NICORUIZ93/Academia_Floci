...
export const COURSE_MODULES: CourseModule[] = [
  m(0,
    'Instalación y primeros pasos con Floci',
    'Primeros pasos',
    ...
    [
      ...
      'Configura las credenciales: eval $(floci env) — verifica con aws sts get-caller-identity',
      'Inicia Floci Azure: floci az start — verifica con floci az status',
      'Inicia Floci GCP: docker run -p 4588:4588 floci/floci-gcp:latest — verifica que responde en localhost:4588',
      'Detén y reinicia cada emulador y comprueba que el estado no persiste por defecto',
    ],
    ['¿Qué diferencia hay entre hablar con AWS real y con Floci?', '¿Qué hace eval $(floci env) exactamente?', '¿Qué ocurre con tus datos cuando detienes el emulador?'],
    ['STS'],
  ...
  m(1,
    ...
    [
      ...
      'Azure Blob: eval $(floci az env) — crea un contenedor con az storage container create --name mi-contenedor --connection-string $AZURE_STORAGE_CONNECTION_STRING',
  ...
  m(2,
    ...
    [
      ...
      'AWS SQS: envía un mensaje — aws sqs send-message --queue-url $(aws sqs get-queue-url --queue-name mi-cola --query QueueUrl --output text) --message-body "Hola Floci"',
      ...
      'Azure Service Bus: eval $(floci az env) — crea una cola: az servicebus queue create --name mi-cola --namespace-name floci',
  ...
  m(3,
    ...
    [
      ...
      'Azure Cosmos DB: conéctate con el SDK de Python o Node.js apuntando a Floci-az (puerto 4577)',
    ],
  ...
  m(4,
    ...
    [
      ...
      'Azure Key Vault: eval $(floci az env) — crea un secreto y léelo con az keyvault secret set/show',
    ],
  ...
  m(5,
    ...
    'Escribe y despliega funciones que se ejecutan sin gestionar servidores. Lambda en AWS ejecuta código real en contenedores Docker. Azure Functions en Floci-az soporta HTTP y Timer triggers.',
    ['Handler function', 'Event payload', 'Context', 'Cold start', 'Concurrencia reservada', 'Lambda Layers'],
    [
      'Escribe una función Python: crea handler.py con def handler(event, context): return {"statusCode": 200, "body": "Hola Floci"}',
      'Empaqueta la función: zip function.zip handler.py',
      'Despliega en Floci: aws lambda create-function --function-name mi-funcion --runtime python3.12 --handler handler.handler --role arn:aws:iam::000000000000:role/lambda-role --zip-file fileb://function.zip',
      ...
      'Azure Functions (Floci-az): crea un archivo function.json con type "httpTrigger" y despliega apuntando a http://localhost:4577',
  ...
  m(7,
    ...
    [
      ...
      'Azure Event Hubs (Floci-az): eval $(floci az env) — crea un Event Hub namespace y envía eventos por AMQP en puerto 5672',
    ],
  ...
  m(9,
    ...
    'Floci corre PostgreSQL real cuando creas una instancia RDS. La única diferencia con AWS real es el endpoint. Aprende cuándo elegir SQL sobre NoSQL.',
    ...
    ['¿Cuándo elegir DynamoDB sobre RDS y viceversa?', '¿Qué diferencia hay entre una instancia RDS en Floci y un PostgreSQL en Docker simple?', '¿Qué es una migration y por qué es importante?'],
    ['RDS'],
  ...
  m(10,
    ...
    'Empaqueta tu aplicación en una imagen Docker, publícala en ECR y ejecútala en ECS. Floci corre contenedores Docker reales.',
    ...
    [
      ...
      'Crea un repositorio ECR en Floci: aws ecr create-repository --repository-name mi-api',
      ...
      'EKS real (Floci): aws eks create-cluster --name dev-cluster --role-arn arn:aws:iam::000000000000:role/eks-role && aws eks update-kubeconfig --name dev-cluster && kubectl run nginx --image=nginx:alpine',
    ],
  ...
  m(12,
    ...
    [
      ...
      'Crea la state machine en Floci: aws stepfunctions create-state-machine --name FlujoTareas --definition file://maquina.json --role-arn arn:aws:iam::000000000000:role/sfn-role',
  ...
  m(13,
    ...
    [
      ...
      'Produce y consume mensajes con kafka-console-producer y kafka-console-consumer apuntando al endpoint del broker MSK en Floci',
      'GCP Managed Kafka (Floci-gcp): levanta el emulador y conéctate al broker en localhost:4588 con kafka-console-producer.sh',
  ...
  m(14,
    ...
    'Cognito / Auth',
    'Experto', '3 h', '#2f6f9f',
    'Implementa registro, login y autorización de usuarios sin construir tu propio sistema de autenticación. Cognito en Floci soporta flujos OAuth 2.0 completos.',
    ...
    [
      ...
      'Confirma el usuario (Floci lo confirma automáticamente o usa admin-confirm): aws cognito-idp admin-confirm-sign-up --user-pool-id <pool-id> --username alice@ejemplo.com',
  ...
  m(15,
    ...
    'Athena / BigQuery',
    'Experto', '3 h', '#e85d4a',
    'Consulta terabytes de datos en S3 con SQL sin moverlos ni cargarlos en una base de datos. Athena en Floci usa DuckDB como motor real.',
  ...
  m(16,
    ...
    'Integra modelos de IA generativa y procesamiento de documentos. Floci emula Bedrock con stubs deterministas — aprende a probar contratos de IA sin depender de un modelo real.',
    ...
    [
      ...
      'Observa que Floci devuelve una respuesta stub determinista (misma respuesta para mismo input)',
    ...
    ['¿Qué diferencia hay entre un stub y un mock?', '¿Qué contratos puedes probar localmente y cuáles necesitan el modelo real?', '¿Por qué Floci usa stubs deterministas para IA en vez de modelos reales?'],
  ...
  m(17,
    ...
    [
      ...
      'Azure: implementa la misma API en Azure Functions + Service Bus + Cosmos DB + Blob Storage (Floci-az)',
      'Azure: usa eval $(floci az env) y az CLI para crear todos los recursos',
      'GCP: implementa los endpoints de solo lectura usando Firestore + Cloud Storage + Pub/Sub (Floci-gcp)',
    ...
    'API completa con los mismos endpoints funcionando en AWS (Floci), Azure (Floci-az) y GCP (Floci-gcp).',
    ['aws', 'azure', 'gcp']
  ),
];
...
export const QUIZ = [
  ...
  { question: '¿En qué puerto escucha el emulador de Azure (floci-az)?', options: ['4566', '4577', '4588'], answer: 1 },
  { question: '¿Qué prueba sigue siendo necesaria después de validar todo con Floci?', options: ['Ninguna, Floci es idéntico a AWS', 'Prueba final contra la nube real', 'Solo linting del código'], answer: 1 }
];
...
export const CLOUD_COMPARISON: CloudComparison[] = [
  ...
  { categoria: 'Cómputo serverless', descripcion: 'Ejecuta código sin aprovisionar servidores.', aws: 'Lambda (Docker real en Floci)', azure: 'Azure Functions (HTTP + Timer)', gcp: 'Cloud Functions / Cloud Run', modulo: 5 },
  ...
  { categoria: 'Base de datos relacional', descripcion: 'PostgreSQL/MySQL gestionado con backups automáticos.', aws: 'RDS (PostgreSQL real en Floci)', azure: 'Azure SQL / PostgreSQL Flexible', gcp: 'Cloud SQL / AlloyDB', modulo: 9 },
  { categoria: 'Contenedores gestionados', descripcion: 'Corre contenedores con orquestación sin gestionar nodos.', aws: 'ECS / EKS (Docker real en Floci)', azure: 'AKS / Container Apps', gcp: 'GKE Autopilot / Cloud Run', modulo: 10 },
  ...
  { categoria: 'Consultas SQL analíticas', descripcion: 'SQL sobre datos en object storage sin moverlos.', aws: 'Athena (DuckDB en Floci)', azure: 'Synapse Serverless', gcp: 'BigQuery', modulo: 15 },
  ...
  { categoria: 'IA generativa (LLMs)', descripcion: 'Accede a modelos de lenguaje grande con API REST.', aws: 'Bedrock Runtime (stub en Floci)', azure: 'Azure OpenAI Service', gcp: 'Vertex AI / Gemini API', modulo: 16 },
  { categoria: 'OCR / Extracción de documentos', descripcion: 'Extrae texto y datos de imágenes y PDFs.', aws: 'Textract (Floci)', azure: 'Document Intelligence', gcp: 'Document AI', modulo: 16 },
  { categoria: 'Registro de imágenes Docker', descripcion: 'Almacena y versiona imágenes de contenedor privadas.', aws: 'ECR (Floci)', azure: 'Container Registry', gcp: 'Artifact Registry', modulo: 10 },
  { categoria: 'Caché en memoria', descripcion: 'Acelera lecturas con Redis real gestionado.', aws: 'ElastiCache (Redis real en Floci)', azure: 'Azure Cache for Redis', gcp: 'Memorystore', modulo: 9 },
  { categoria: 'Configuración externalizada', descripcion: 'Parámetros de app sin reiniciar ni tocar código.', aws: 'SSM Parameter Store', azure: 'App Configuration (Floci-az)', gcp: 'Runtime Configurator', modulo: 4 },
  { categoria: 'Correo electrónico transaccional', descripcion: 'Envía y recibe correos desde tu aplicación.', aws: 'SES (Floci)', azure: 'Communication Services', gcp: 'Gmail API (Workspace)', modulo: 7 },
  { categoria: 'DNS gestionado', descripcion: 'Gestiona registros DNS con alta disponibilidad.', aws: 'Route 53 (Floci)', azure: 'Azure DNS', gcp: 'Cloud DNS', modulo: 10 },
];

export const AZURE_GROUPS: AltCloudGroup[] = [
  { nombre: 'Identidad y seguridad', color: '#0078d4', descripcion: 'Gestiona acceso, secretos y cumplimiento — disponible en Floci-az (puerto 4577).', servicios: ['Key Vault (Secrets/Keys/Certs)', 'Entra ID / Azure AD', 'Azure RBAC', 'Azure Policy'] },
  { nombre: 'Almacenamiento y datos', color: '#00b4d8', descripcion: 'Persiste objetos, tablas, documentos y colas — disponible en Floci-az.', servicios: ['Blob Storage', 'Queue Storage', 'Table Storage', 'Cosmos DB (SQL/Mongo/Cassandra/Gremlin)'] },
  { nombre: 'Mensajería y eventos', color: '#7209b7', descripcion: 'Desacopla, enruta y transmite — disponible en Floci-az (AMQP: 5672/5673).', servicios: ['Service Bus (Queues + Topics)', 'Event Hubs (AMQP + Kafka + REST)'] },
  { nombre: 'Cómputo', color: '#e63946', descripcion: 'Ejecuta código sin gestionar servidores — disponible en Floci-az.', servicios: ['Azure Functions (HTTP Trigger)', 'Azure Functions (Timer Trigger)', 'AKS (k3s/mocked)'] },
  { nombre: 'Configuración', color: '#2ec4b6', descripcion: 'Externaliza configuración de la app — disponible en Floci-az.', servicios: ['App Configuration (con Labels)', 'Azure SQL (en progreso)'] },
];

export const GCP_GROUPS: AltCloudGroup[] = [
  { nombre: 'Almacenamiento', color: '#34a853', descripcion: 'Persiste objetos y documentos — disponible en Floci-gcp (puerto 4588).', servicios: ['Cloud Storage (REST JSON/XML)', 'Firestore (gRPC)', 'Datastore (REST JSON)'] },
