...
import {
  ArrowLeft, Award, BookOpen, Boxes, Check, ChevronLeft, ChevronRight, CircleHelp, CloudCog,
...
@Component({
  selector: 'app-cloud-classic',
  imports: [CommonModule, FormsModule, RouterLink, LucideAngularModule],
  templateUrl: './cloud-classic.html',
  styleUrl: './cloud-classic.scss',
  encapsulation: ViewEncapsulation.None
})
export class CloudClassicComponent implements OnInit {
  readonly icons = { ArrowLeft, Award, BookOpen, Boxes, Check, ChevronLeft, ChevronRight, CircleHelp, CloudCog, Code2, Copy, Database, Download, FileCheck2, FileText, FlaskConical, Focus, Gauge, GraduationCap, LayoutDashboard, Library, ListChecks, LockKeyhole, Menu, MessageSquareText, Network, PartyPopper, Play, Search, ServerCog, Settings2, Sparkles, Target, Terminal, Trophy, X };
  // Getters en vez de campos inicializados en construcción: en el bundle de pruebas
  // (Vitest/esbuild con múltiples specs), leer estas constantes de course-data.ts
  // como valor de campo en el momento de construir la instancia podía capturar el
  // binding antes de que ese chunk terminara de evaluarse. Como getter se leen en
  // el momento del render, ya con el módulo completamente cargado.
  get modules(): CourseModule[] { return COURSE_MODULES; }
  get serviceGroups() { return SERVICE_GROUPS; }
  get quiz() { return QUIZ; }
  get azureGroups(): AltCloudGroup[] { return AZURE_GROUPS; }
  get gcpGroups(): AltCloudGroup[] { return GCP_GROUPS; }
  get cloudComparison() { return CLOUD_COMPARISON; }
...
Console.WriteLine("Implementa el cliente SDK siguiendo el módulo ${project.module} y verifica con la CLI.");`;
  ...
  async openDocument(document: LibraryDocument): Promise<void> {
    ...
    try {
      const response = await fetch(this.resolveContentPath(document.path));
...
public class App {
  ...
  /** Las rutas de contenido en manifest.json y en los fetch directos son relativas a la raíz del sitio,
   *  no a la ruta del Router actual (ej. /curso/cloud) — se normalizan a absolutas antes de pedirlas. */
  private resolveContentPath(path: string): string {
    return path.startsWith('/') ? path : `/${path}`;
  }
  private async loadLibrary(): Promise<void> {
    try {
      const response = await fetch(this.resolveContentPath('content/manifest.json'));
  ...
  private async loadLesson(id: number): Promise<void> {
    ...
    try {
      const response = await fetch(this.resolveContentPath(`content/lecciones/modulo-${id}.md`));
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  private async loadServiceStudies(): Promise<void> {
    try {
      const studies = await fetch(this.resolveContentPath('content/service-study.json')).then(response => response.json()) as ServiceStudy[];
